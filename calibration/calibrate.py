...
from michael.domains import Routing  # noqa: E402
...
def probe(query: str, target: tuple[str, str] | None) -> Probe:
    """Score a query once, with the threshold effectively disabled."""
    result = search(query, routing=UNFILTERED, top_k=TOP_K, min_score=0.0)
    best = result.provisions[0].score if result.provisions else 0.0

    target_score: float | None = None
    target_rank: int | None = None
    if target is not None:
        citation, section = target
        for rank, provision in enumerate(result.provisions, start=1):
            if provision.citation == citation and provision.section_number == section:
...
def main() -> int:
    ...
    good = [
        probe(case["query"], (case["citation"], case["section"])) for case in data["known_good"]
    ]
    print("Probing known-absent queries...")
    absent = [probe(case["query"], None) for case in data["known_absent"]]
    ...
    for case, p in zip(data["known_good"], good, strict=True):
        ...
        where = f"{case['citation'][:38]} s {case['section']}"
